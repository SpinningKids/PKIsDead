...
template <class T> void panswap(T &x, T &y) {
  T z = x;
  x = y;
  y = z;
}
...
struct Vector3
{
  ...
  float operator | (Vector3 &b) const {
	  return (float)(((*this)*b)/sqrt((*this)*(*this)*(b*b))); // il caso nullo e' molto raro, dovresti gestirlo anche in / a questo punto!
  }
...
/* // c'e' il copy constructor apposta
...
*/
  ...
  float LengthSquare() const // diobono, si scrive Length!
  {
    return (x*x + y*y + z*z);
  }
  ...
  Vector3 Reflected(const Vector3 &n) const // cambiata di nome
  {
...
public:
	...
	explicit Matrix4(const Vector3 &v) // quando fai queste porcherie metti sempre explicit, altrimenti
    // ti permette di fare Matrix M = v; dove v e' un vettore.
	{
		SetTranslation(v);
	}
...
//    wx = m[12]; wy = m[13]; wz = m[14]; dw = m[15];
  ...
  void Inverse()
	...
	void Translate(const Vector3 &v) // O cambia la matrice O ritorna qualcosa. Non tutti e due, plz.
	{
		wx+=v.x;
		wy+=v.y;
		wz+=v.z;
  ...
  inline void GetInverseTranslateVec(Vector3* pVect)
  {
    pVect->x = pVect->x - wx;
		pVect->y = pVect->y - wy;
		pVect->z = pVect->z - wz;
	...
	void GetInverseRotateVec( Vector3* pVect )
	{
		Vector3 vec;
  ...
  void SetCamera(float roll, Vector3 position, Vector3 target)
	{
		Vector3 worldup((float)sin(-roll), (float)cos(-roll), 0.0f);    
		Vector3 view;
...
struct Sphere
{
  ...
  inline Sphere()
  {
    ...
    radius = radius_sqrd = 1.0f;
    radius_sqrd = (float)(radius * radius);
  }

  inline void SetRadius(float r)
  {
    radius = fabsf(r); // fabs per gentile concessione della Intel 
    radius_sqrd = (float)(radius * radius);
  }
