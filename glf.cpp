...
|  For commercial usage, please contact me: romka@ut.
==============================================================================
*/

/***************************
 Modified by Aragorn/SpinningKids

 Added the ability to load fonts straight from memory
 (just wrapped around fopen/fread/fclose...)
 Added glfGenLists
 (generates a display list from the loaded font, along with a GLYPHMETRICSFLOAT-
  like structure)

So:
We shuold generate 96 display lists. Starting from 32 (ASCII code for the space) on.
****************************/
...
// Scale factor (the glyphs are scaled by this value in glfGenLists.
// Tweak this value to mimic wglUseFontOutlines generated glyphs' size)
#define SCALEFIT 0.4f
// Additional space between symbols
#define SYM_SPACE 0.2f
...
typedef FILE * (*FOPEN_CALLBACK) (const char *FILENAME, const char *OPENTYPE);
typedef size_t (*FREAD_CALLBACK) (void *DATA, size_t SIZE, size_t COUNT, FILE *STREAM);
typedef int (*FCLOSE_CALLBACK) (FILE *STREAM);


/* ------------- Main variables -------------- */

// file functions callback slots and variables
static FOPEN_CALLBACK fake_fopen;
static FREAD_CALLBACK fake_fread;
static FCLOSE_CALLBACK fake_fclose;
...
FILE * memory_fopen (const char *FILENAME, const char *OPENTYPE)
{
    return (FILE*) FILENAME;
}

size_t memory_fread (void *DATA, size_t SIZE, size_t COUNT, FILE *STREAM)
{
    int size = COUNT*SIZE;
    GLF_MEMFILE *memfile = (GLF_MEMFILE *)STREAM;

    if (memfile->pos + size >= memfile->length)
	size = memfile->length - memfile->pos;
    
    memcpy(DATA, (char *)memfile->data+memfile->pos, size);
    memfile->pos += size;
    
    return(size);
}

int memory_fclose (FILE *STREAM)
{
    return 0;
}
...
void glfSetMemoryLoad()
{
    fake_fopen  = memory_fopen;
    fake_fread  = memory_fread;
    fake_fclose = memory_fclose;
}
...
static int ReadFont(char *font_name, struct glf_font *glff)
{
	...
	fontf = fake_fopen(font_name, "rb");
	if (fontf == NULL) return GLF_ERROR;

	fake_fread(buffer, 3, 1, fontf);
	buffer[3] = 0;
	...
	fake_fread(glff->font_name, 96, 1, fontf);
	glff->font_name[96] = 0;

	fake_fread(&glff->sym_total, 1, 1, fontf); /* Read total symbols in font */

	for (i=0; i<MAX_FONTS; i++) glff->symbols[i] = NULL;

	for (i=0; i<28; i++) fake_fread(&temp, 1, 1, fontf);  /* Read unused data */

	/* Now start to read font data */
  
	for (i=0; i<glff->sym_total; i++)
	{
		fake_fread(&code, 1, 1, fontf);  /* Read symbol code   */
		fake_fread(&verts, 1, 1, fontf); /* Read vertexs count */
		fake_fread(&fcets, 1, 1, fontf); /* Read facets count  */
		fake_fread(&lns, 1, 1, fontf);   /* Read lines count   */
		...
		for (j=0; j<verts; j++)
		{
			fake_fread(&tempfx, 4, 1, fontf);
			fake_fread(&tempfy, 4, 1, fontf);
		...
		for (j=0; j<fcets; j++)
			fake_fread(&glff->symbols[code]->fdata[j*3], 3, 1, fontf);
		for (j=0; j<lns; j++)
			fake_fread(&glff->symbols[code]->ldata[j], 1, 1, fontf);
	}
	fake_fclose(fontf);
	return GLF_OK;
}
...
*/
int glfLoadFontFromMemory(unsigned char *font_data)
{
    return 0;
}
...
// Generates the display lists corresponding to current font's glyphs ranging
// from ASCII 32 to 32+96-1 (most printable characters)
// return the base display list
GLuint glfGenLists(GLYPHMETRICSFLOAT *gmf)
{
    GLuint base = glGenLists(96);

    for (GLuint i=32; i<(32+96); i++) {
	if (fonts[curfont]->symbols[i] != NULL) {
	    glNewList(base + i - 32, GL_COMPILE);
	    glPushMatrix();
	    glScalef(SCALEFIT,SCALEFIT,0);
	    // 65 is the code for capital ``A''
  	    glTranslatef(-fonts[curfont]->symbols[i]->leftx, -fonts[curfont]->symbols[65]->topy, 0);
  	    glfDrawSolidSymbol((char)i);
	    glPopMatrix();
	    gmf[i-32].gmfCellIncX  = (fonts[curfont]->symbols[i]->rightx - fonts[curfont]->symbols[i]->leftx + SYM_SPACE) * SCALEFIT;
	    gmf[i-32].gmfBlackBoxY = (fonts[curfont]->symbols[i]->bottomy - fonts[curfont]->symbols[i]->topy) * SCALEFIT;
	    glTranslatef(gmf[i-32].gmfCellIncX, 0, 0);
	    glEndList();
	} else if ((char) i == ' ') {
	    glNewList(base + i - 32, GL_COMPILE);
  	    glTranslatef(SpaceSize*.5*SCALEFIT, 0, 0);
	    gmf[i-32].gmfCellIncX  = SpaceSize*.5*SCALEFIT;
	    gmf[i-32].gmfBlackBoxY = SpaceSize*.5*SCALEFIT;
	    glEndList();
	}
    }
